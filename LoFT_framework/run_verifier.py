...
def main():
    ...
    if training_args.do_predict:
        ...
        for pred_logits in predictions:
            entailed_score = math.exp(pred_logits[1]) / sum([math.exp(pred_logits[1]), math.exp(pred_logits[0])])
            entailed_score = round(entailed_score, 4)
            entailed_weights.append(entailed_score)

        predictions = np.argmax(predictions, axis=1)
        
        inference_file = "LoFT_predict/inference_verifier_input.json"
        output_inference_file = "LoFT_predict/inference_with_verification_result.json"
        data = json.load(open(inference_file, "r"))
        output_data = []
        for index, item in enumerate(predictions):
            cur_example = data[index]
            cur_example["verification_label"] = "entailed" if item == 1 else "refuted"
            cur_example["verification_score"] = entailed_weights[index]
            output_data.append(cur_example)
        
        json.dump(output_data, open(output_inference_file, "w"), indent = 4)
