...
class PGAgent(nn.Module):
    ...
    def compute_trajectory_actions_prob(self, trajectories: List[Trajectory], return_info=False) -> torch.Tensor:
        ...
        # (batch_size, max_action_len) !!! here tgt_action_id select one action from action_log_probs'memory_size, lead tgt_action_log_probs to be (batch_size, max_action_len)
    ...
    def forward(self, trajectories: List[Trajectory], entropy=False, return_info=False): # !!! used by learner.py calculate prob
    ...
    def sample(
        ...
        while True: # !!! every loop, all env in the batch will generate an action
            ...
            for env_id, (env, action_t) in enumerate(zip(active_envs, sampled_action_t_id.tolist())):
                ...
                if env.done:
                    ...
                    if ob_t.valid_action_indices:
                        ...
                        # is finished # !!! don't add to completed_envs
                        has_completed_sample = True

            if not new_active_env_pos: # !!! when all env in the batch finish generate a program, new_active_env_pos will be empty and break
                break

            if has_completed_sample: # !!! if some env in the batch finish generate a program
                # need to perform slicing !!! drop the env finish sample, only leave unfinished ones
            ...
            else: # !!! if no env in the batch finish generate a program, then only update the state for next step
                state_tm1 = state_t
    ...
    def new_beam_search(self, environments, beam_size, use_cache=False, return_list=False, # !!! used by actor.py
    ...
    def decode_examples(self, environments: List[QAProgrammingEnv], beam_size, batch_size=32): # !!! used by experiment.py test(), evaluator.py evaluate() 
        decode_results = []
    ...
    def sample_action(self, logits, valid_action_mask, return_log_prob=False):
        ...
        # (batch_size, 1) !!! here sample one from multinomial distribution 
    ...
    @classmethod
    def load(cls, model_path, gpu_id=-1, default_values_handle=None, **kwargs): # !!! this func seems to be useless orz, it only used once
