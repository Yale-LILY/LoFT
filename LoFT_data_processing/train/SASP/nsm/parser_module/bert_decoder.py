...
class BertDecoder(DecoderBase):
    def __init__(
        ...
        **kwargs
        ...
        # self.att_vec_linear = nn.Linear(encoder_output_size + hidden_size*2, hidden_size, bias=False)
    ...
    def step(self, x: Union[List[Observation], Observation], state_tm1: DecoderState, context_encoding: Dict):
        ...
        # (batch_size, hidden_size)
        # att_t = torch.tanh(self.att_vec_linear(torch.cat([inner_output_t, node_embedding, ctx_t], 1)))  # E.q. (5)
        ...
        # print('mem_logits', mem_logits.size(), max(mem_logits[0]), min(mem_logits[0]), mem_logits)
        # print('valid_action_scores', x.valid_action_scores.size(), max(x.valid_action_scores[0]), min(x.valid_action_scores[0]), x.valid_action_scores)
        mem_logits = mem_logits + x.valid_action_scores # eq(2) in our paper
