...
def normalize_probs(p_list, epoch):
    smoothing = 0.1*pow(0.5, epoch)
...
class ReplayBuffer(object):
            ...
            if env.name in programs:
                ...
                for program_str in program_str_list:
                    ...
                    if traj is not None and traj.reward > 0.:
                        ...
                        if not found:
                            n_found += 1 # only add once when found is false (for the first program satisfy traj.reward > 0, n_found is still false)
                        found = True
    ...
    def replay(self, environments, n_samples=1, use_top_k=False, truncate_at_n=0, replace=True,
               consistency_model=None, constraint_sketches=None, debug_file=None, epoch=0):
        ...
        for env_name, samples in env_sample_dict.items():
            ...
            if use_top_k:
                ...
                if consistency_model:
                    ...
                    p_samples = normalize_probs([sample.prob for sample in samples], epoch)
